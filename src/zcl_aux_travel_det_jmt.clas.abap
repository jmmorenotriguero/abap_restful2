CLASS zcl_aux_travel_det_jmt DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_travel_reported      TYPE TABLE FOR REPORTED
         z_i_travel_jmt,
           tt_booking_reported     TYPE TABLE FOR REPORTED
               z_i_booking_jmt,
           tt_supplements_reported TYPE TABLE FOR REPORTED
                                     z_i_booksuppl_jmt.
...
CLASS ZCL_AUX_TRAVEL_DET_JMT IMPLEMENTATION.
  ...
  METHOD calculate_price.
    ...
    GROUP BY ls_booking-travel_id INTO DATA(lv_travel_key).
      ASSIGN lt_read_travel[ KEY entity COMPONENTS travel_id
      = lv_travel_key ]
      TO FIELD-SYMBOL(<ls_travel>).
      LOOP AT GROUP lv_travel_key INTO
      DATA(ls_booking_result)
      GROUP BY ls_booking_result-currency_code INTO
      DATA(lv_curr).
    ...
    GROUP BY ls_booking_suppl-travel_id INTO
    lv_travel_key.
      ASSIGN lt_read_travel[ KEY entity COMPONENTS travel_id
      = lv_travel_key ] TO <ls_travel>.
      LOOP AT GROUP lv_travel_key INTO
      DATA(ls_supplements_result)
      GROUP BY ls_supplements_result-currency_code INTO
      lv_curr.
    ...
    ENTITY Travel
    UPDATE FROM VALUE #( FOR ls_travel_bo IN
    lt_read_travel (
    travel_id =
    ls_travel_bo-travel_id
    total_price =
