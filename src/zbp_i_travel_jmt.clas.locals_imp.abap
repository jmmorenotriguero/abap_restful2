...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  cl_abap_context_info=>get_user_technical_name( ) EQ 'CB9980004208'
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF z_i_travel_jmt IN LOCAL MODE
    ENTITY Travel
    ...
    READ ENTITIES OF z_i_travel_jmt IN LOCAL MODE
    ENTITY Travel
    FIELDS ( agency_id
             customer_id
             begin_date
    ...
    LOOP AT lt_travel ASSIGNING FIELD-SYMBOL(<ls_travel>).
      ...
      APPEND VALUE #( travel_id = <ls_travel>-travel_id
                      %msg      = new_message( id = 'Z_MC_TRAVEL_JMT'
                                               number = '006'
                                               v1 = lv_travel_msg
  ...
  METHOD createTravelByTemplate.
    ...
    DATA lt_create_travel TYPE TABLE FOR CREATE z_i_travel_jmt\\Travel.

    SELECT MAX( travel_id ) FROM ztb_travel_jmt
  ...
  METHOD validateDate.
    ...
    READ ENTITY z_i_travel_jmt\\Travel
...
CLASS lsc_Z_I_TRAVEL_JMT DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_Z_I_TRAVEL_JMT IMPLEMENTATION.

  METHOD save_modified.

    DATA: lt_travel_log   TYPE STANDARD TABLE OF ztb_log_jmt,
          lt_travel_log_u TYPE STANDARD TABLE OF ztb_log_jmt.
    ...
    IF NOT create-travel IS INITIAL.
      ...
      LOOP AT lt_travel_log ASSIGNING FIELD-SYMBOL(<ls_travel_log>).
        ...
        <ls_travel_log>-changing_operation = lsc_z_i_travel_jmt=>create.
        READ TABLE create-travel WITH TABLE KEY entity
        COMPONENTS travel_id = <ls_travel_log>-travel_id
        INTO DATA(ls_travel).
        IF sy-subrc EQ 0.
          IF ls_travel-%control-booking_fee EQ
          cl_abap_behv=>flag_changed.
            <ls_travel_log>-changed_field_name =
            'booking_fee'.
            <ls_travel_log>-changed_value = ls_travel-booking_fee.
            <ls_travel_log>-user_mod = lv_user.
            TRY.
                <ls_travel_log>-change_id =
    ...
    IF NOT update-travel IS INITIAL.
      ...
      LOOP AT update-travel INTO DATA(ls_update_travel).
        ...
        <ls_travel_log_bd>-changing_operation = lsc_z_i_travel_jmt=>update.
        IF ls_update_travel-%control-customer_id EQ
        cl_abap_behv=>flag_changed.
          <ls_travel_log_bd>-changed_field_name =
          'customer_id'.
          <ls_travel_log_bd>-changed_value =
          ls_update_travel-customer_id.
          <ls_travel_log_bd>-user_mod = lv_user.
          TRY.
              <ls_travel_log_bd>-change_id =
              cl_system_uuid=>create_uuid_x16_static( ).
            CATCH cx_uuid_error.
          ENDTRY.
    ...
    IF NOT delete-travel IS INITIAL.
      ...
      LOOP AT lt_travel_log ASSIGNING FIELD-SYMBOL(<ls_travel_log_del>).
        ...
        <ls_travel_log_del>-changing_operation = lsc_z_i_travel_jmt=>delete.
        <ls_travel_log_del>-user_mod = lv_user.
        TRY.
            <ls_travel_log_del>-change_id =
            cl_system_uuid=>create_uuid_x16_static( ).
          CATCH cx_uuid_error.
        ENDTRY.
    ...
    IF NOT lt_travel_log_u IS INITIAL.
      INSERT ztb_log_jmt FROM TABLE @lt_travel_log_u.
    ENDIF.
